...
2. **Grammar carrier lengths (問題7–9)** — extract text from the booklet PDFs (they have a text layer) and count Japanese characters in each stem / cloze body. Measured across 07/2023–12/2025:
   - **問題7**: ~43 JP chars average per stem (median ~41; most items 33–54; occasional short ~25 and long ~70+ dialogue stems). Mock stems averaging under ~35, or a run of under-30 stems, are out of band — lengthen the situation (setting label, subordinate clause, short dialogue), not the tested form.
...
3. **Distractor Patterns**: Examine how official items create plausible distractors (e.g. 近義語 traps in 問題5, 誤用 types in 問題6, condition traps in 問題14).
4. **Furigana & Vocab Notes**: Benchmark `（注1）` explanations against official formatting in reading passages.
5. **Listening Script Phrasing**: Compare spoken option length and dialogue turns against the script PDFs in `refs/JLPT/`.
