...
3. **Comprehensive & Concise Explanation (詳細・簡潔な解説)**:
   ...
   - **Full Option-by-Option Distractor Analysis (`全選択肢の個別解説・誤答分析`)**: Clear, concise, concrete explanation for EVERY choice (1, 2, 3, 4) explaining why it is correct or why it fails (contrary to text, unmentioned, wrong collocation, incorrect particle/verb form, wrong timing in audio). Exactly ONE option is tagged `[正解]` matching the official answer key, while all distractor options are tagged `[不正解]`. Never can all choices be labeled `[不正解]`, nor can an answer key be mismatched or defaulted. Generic placeholder sentences like "選択肢 1 は文脈・文法制約に合致しません" are strictly prohibited.
