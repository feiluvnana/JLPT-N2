...
and a **seed** no previous test used (`logs/ledger.json` records them).

---

Generate a complete JLPT N2 mock exam as test `⟨test_id⟩` with seed `⟨seed⟩`
in this repository.
...
2. **Use exactly the id `⟨test_id⟩` and seed `⟨seed⟩`**, re-harvest
   `logs/seeds.json` fresh for this test (skip the web blend only if you have
   no web access, and say so), and verify the blend report before authoring.
