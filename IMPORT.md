...
4. **Model answer (FINAL STEP)**, once all 101 keys are reconciled and the
   gate is green: author `詳細解説.json` (`exam-model-answer`), translate it
   into the languages `GENERATE.md` step 4b declares — that table is the only
   copy of the list (`exam-answer-translation`) — then run
   `make model-answer imported-⟨slug⟩` last.
